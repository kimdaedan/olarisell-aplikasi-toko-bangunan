...
<body class="bg-gray-100">
    ...
    <form action="{{ url('/products') }}" method="GET" class="flex">
        <input type="text" name="search" placeholder="Cari produk..." class="border border-gray-300 rounded p-2 w-full mr-2">
        <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded">Cari</button>
    </form>
</div>
                    <a href="/kasir" class="bg-green-500 text-white px-4 py-2 rounded">Kasir</a>
                    <button onclick="toggleForm()" class="bg-red-500 text-white px-4 py-2 rounded ml-2">Tambah</button>
                </div>
            </div>
                    ...
                    <tbody>
                        @foreach ($products as $product)
                            <tr>
                                ...
                                <td class="border border-gray-300 p-2">
                                    <a href="{{ url('/products/' . $product->id . '/edit') }}" class="bg-blue-500 text-white px-4 py-1 rounded">Edit</a>
                                    <form action="{{ url('/products/' . $product->id) }}" method="POST" class="inline" onsubmit="confirmDelete(event);">
                                        @csrf
                                        @method('DELETE')
